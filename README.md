...
A simple ansible role for running [Prowlarr](https://prowlarr.tv/) via Docker compose using the [LinuxServer.io Prowlarr image](https://docs.linuxserver.io/images/docker-prowlarr).
...
- hosts: prowlarr
  ...
  vars:
    ...
    prowlarr_docker_image: "lscr.io/linuxserver/prowlarr:latest"
    ...
    prowlarr_docker_volumes:
      - "{{ prowlarr_docker_base_directory }}/config:/config"
      - '/path/to/tv:/tv'
...
- hosts: prowlarr
  ...
  vars:
    ...
    prowlarr_docker_service_name: "prowlarr"

    # User that will own the compose files
    prowlarr_docker_user: "{{ ansible_user }}"
    ...
    prowlarr_docker_container_name: 'prowlarr'
    ...
    prowlarr_docker_mem_limit: '250mb'
